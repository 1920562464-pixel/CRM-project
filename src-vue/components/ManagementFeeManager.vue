<template>
  <div class="space-y-6">
    ...
    <!-- Tab 切换 -->
    <div class="flex bg-gradient-to-br from-slate-50 to-slate-100 p-1.5 rounded-xl shadow-sm border border-slate-200">
      <button
        v-for="tab in tabs"
        :key="tab.id"
      ...
      >
        <component :is="tab.icon" :size="16" :class="activeTab === tab.id ? 'text-white' : ''" />
        {{ tab.label }}
        <span v-if="activeTab === tab.id" class="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-8 h-0.5 bg-white/50 rounded-full"></span>
      </button>
    </div>
...
import { Briefcase, DollarSign, CheckCircle, TrendingUp, Plus, Trash2, AlertCircle, Users, Stethoscope } from 'lucide-vue-next'
import Toast from './shared/Toast.vue'
import Modal from './shared/Modal.vue'
import CoachSettlement from './CoachSettlement.vue'
import DoctorSettlement from './DoctorSettlement.vue'
...
const tabs: Tab[] = [
  { id: 'management', label: '管理费', icon: Briefcase },
  { id: 'coach', label: '教练结算', icon: Users },
  { id: 'doctor', label: '医生结算', icon: Stethoscope }
]
