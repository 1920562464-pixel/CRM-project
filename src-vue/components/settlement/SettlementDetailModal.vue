<template>
  <Teleport to="body">
    <div v-if="show" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
      <div v-if="settlement" class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <!-- 头部 -->
        <div class="sticky top-0 bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-4 flex items-center justify-between">
          <div class="flex items-center gap-4">
            ...
            <div class="text-white">
              ...
              <p class="text-sm opacity-90">{{ settlement.period }} · {{ settlement.department || '运动康复部' }}</p>
            </div>
          </div>
        ...
        <div class="p-6 space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="bg-gradient-to-br from-amber-50 to-orange-50 rounded-xl p-4 border border-amber-200">
              <div class="text-sm text-slate-600 mb-1">服务次数</div>
              <div class="text-2xl font-bold text-amber-600">{{ settlement.serviceCount || settlement.totalUsers * 8 }}次</div>
            </div>
            <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl p-4 border border-purple-200">
              <div class="text text-slate-600 mb-1">平均评分</div>
          ...
          <div class="bg-slate-50 rounded-xl p-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div class="bg-white rounded-lg p-4">
                <div class="flex items-center justify-between mb-3">
                  ...
                  <span class="px-2 py-1 bg-green-100 text-green-800 rounded text-xs font-medium">¥{{ settlement.newUserRate || 200 }}/人</span>
                </div>
              ...
              <div class="bg-white rounded-lg p-4">
                <div class="flex items-center justify-between mb-3">
                  <span class="text-sm font-medium text-slate-700">老用户收入</span>
                  <span class="px-2 py-1 bg-blue-100 text-blue-800 rounded text-xs font-medium">¥{{ settlement.oldUserRate || 100 }}/人</span>
                </div>
                <div class="flex items-end justify-between mb-2">
                  <div>
                    <div class="text-2xl font-bold text-blue-600">¥{{ settlement.oldUserAmount.toLocaleString() }}</div>
                    <div class="text-sm text-slate-500">{{ settlement.oldUsers }} 位老用户</div>
                  </div>
            ...
            <div class="bg-white rounded-lg p-4 mt-4">
              <div class="flex items-center justify-between mb-3">
                <span class="text-sm font-medium text-slate-700">绩效奖金</span>
                <Target :size="18" class="text-amber-500" />
              </div>
              <div class="flex items-center justify-between">
                <div class="text-3xl font-bold text-amber-600">¥{{ settlement.performanceBonus.toLocaleString() }}</div>
                <div class="text-sm text-slate-500">基于服务质量和数量综合计算</div>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div class="bg-gradient-to-r from-slate-50 to-slate-100 px-6 py-4 border-b border-slate-200 flex items-center justify-between">
              <h4 class="font-bold text-slate-900 flex items-center gap-2">
                <List :size="18" class="text-indigo-600" />
                服务记录明细
              </h4>
              <div class="flex items-center gap-2">
                <span class="text-sm text-slate-500">共 {{ serviceRecords.length }} 条服务</span>
                <button class="text-indigo-600 hover:text-indigo-800 text-sm font-medium">导出</button>
              </div>
            </div>
                ...
                <tbody class="divide-y divide-slate-200">
                  <tr v-for="record in serviceRecords" :key="record.id" class="hover:bg-slate-50">
                    ...
                    <td class="px-3 py-3 text-sm text-slate-600">
                      <span class="px-2 py-0.5 bg-indigo-100 text-indigo-800 rounded text-xs whitespace-nowrap">{{ record.serviceType }}</span>
                    </td>
        ...
        <div class="sticky bottom-0 bg-white border-t border-slate-200 px-6 py-4 flex items-center justify-end gap-3">
          ...
          <button
            ...
            class="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 flex items-center gap-2 font-medium transition-colors"
          >
            <CreditCard :size="18" />
            确认发放
          </button>
      ...
      <div v-else class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-12 text-center">
        <p class="text-slate-500">未找到结算数据</p>
      </div>
    </div>
  </Teleport>
</template>
...
import { User, X, BarChart3, TrendingUp, Users, Target, List, Star, CheckCircle, FileText, CreditCard } from 'lucide-vue-next'
...
const props = defineProps<{
  show: boolean
  settlement?: SettlementRecord & { serviceRecords?: ServiceRecord[] }
}>()
...
const serviceRecords = computed<ServiceRecord[]>(() => {
  ...
  const serviceTypes = ['体能训练', '康复指导', '营养咨询', '运动评估', '产后康复', '脊柱矫正']

  const count = props.settlement.serviceCount || props.settlement.totalUsers
  ...
  for (let i = 0; i < Math.min(count, 10); i++) {
    ...
    // 生成干预时长（7-90天）
    const interventionDuration = Math.floor(Math.random() * 84) + 7
