      ...
      <div v-for="rule in rules" :key="rule.id" class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        <div class="grid grid-cols-3 gap-4">
          ...
          <div class="p-4 bg-purple-50 rounded-lg">
            <div class="text-sm text-purple-600 mb-1">上线服务单价</div>
            <div class="text-2xl font-bold text-purple-700">¥{{ rule.onlineRate }}</div>
            <div class="text-xs text-purple-500 mt-1">元/次</div>
          </div>
...
defineProps<{
  rules: DoctorSettlementRule[]
}>()

defineEmits<{
  'save': [rule: DoctorSettlementRule]
  'delete': [id: string]
}>()
</script>
