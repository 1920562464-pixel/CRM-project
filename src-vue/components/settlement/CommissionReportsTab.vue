  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      <div class="group bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-6 text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
        ...
        <div class="text-3xl font-bold mb-1">{{ stats.activeConsultants }}</div>
        <div class="text-blue-100 text-sm">合作顾问</div>
      </div>
      ...
      <div class="group bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-6 text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
        ...
        <div class="text-purple-100 text-sm">服务客户总数</div>
      </div>
      ...
      <div class="group bg-gradient-to-br from-amber-500 to-amber-600 rounded-xl p-6 text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
        ...
        <div class="text-amber-100 text-sm">待发放佣金</div>
      </div>
      ...
      <div class="group bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-6 text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
        ...
        <div class="text-green-100 text-sm">已发放佣金</div>
      </div>
    </div>

    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      <h3 class="text-lg font-semibold text-slate-900 mb-4">月度佣金趋势</h3>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-semibold text-slate-900 flex items-center gap-2">
          <Briefcase class="text-amber-500" :size="20" />
          顾问业绩排行
        </h3>
      ...
      <div class="space-y-3">
        <div
          v-for="(consultant, index) in topConsultants"
          :key="consultant.id"
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <img :src="consultant.avatar" :alt="consultant.name" class="w-10 h-10 rounded-full ring-2 ring-white shadow-sm" />
            <div>
              <span class="font-semibold text-slate-900">{{ consultant.name }}</span>
              <div class="text-xs text-slate-500 mt-0.5">{{ consultant.company }}</div>
            </div>
          </div>
          <div class="text-right">
            <div class="flex items-center justify-end gap-1">
              <span class="text-xl font-bold text-indigo-600">{{ consultant.totalUsers }}</span>
              <span class="text-sm text-slate-600">人</span>
            </div>
            <div class="text-xs text-slate-500 mt-1">累计 ¥{{ consultant.totalCommission.toLocaleString() }}</div>
          </div>
        </div>
      </div>
    </div>
...
import { Users, Clock, DollarSign, BarChart3, Briefcase } from 'lucide-vue-next'
