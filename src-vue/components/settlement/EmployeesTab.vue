      ...
      <table class="w-full">
        <thead class="bg-slate-50 border-b border-slate-200">
          <tr>
            ...
            <th class="px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase">员工信息</th>
        ...
        <tbody class="divide-y divide-slate-200">
          <template v-for="emp in employees" :key="emp.id">
              ...
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  <div class="relative">
                    ...
                    <div v-if="emp.totalUsers > 40" class="absolute -top-1 -right-1 w-4 h-4 bg-amber-400 rounded-full flex items-center justify-center shadow-sm">
                      <Target :size="8" class="text-white" />
                    </div>
                  </div>
                  <div>
                    <div class="font-medium text-slate-900 group-hover:text-indigo-600 transition-colors">{{ emp.name }}</div>
                    <div class="text-xs text-slate-500">{{ emp.phone }}</div>
                  </div>
                </div>
              </td>
              <td class="px-6 py-4">
                <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-sm text-slate-700 bg-slate-100 border border-slate-200">
                  {{ emp.department }}
                </span>
              </td>
            ...
            <tr v-if="expandedEmployees.has(emp.id)">
              <td colspan="7" class="px-6 py-5 bg-gradient-to-br from-slate-50 to-indigo-50/30">
                <div class="space-y-5">
                  <!-- 统计概览 -->
                  <div class="grid grid-cols-4 gap-4 mb-4">
                    <div class="group bg-white rounded-xl p-4 border border-slate-200 shadow-sm">
                    ...
                    <div class="group bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-4 border border-green-200 shadow-sm">
                      <div class="flex items-center justify-between mb-2">
                    ...
                    <div class="group bg-gradient-to-br from-purple-50 to-violet-50 rounded-xl p-4 border border-purple-200 shadow-sm">
                      <div class="flex items-center justify-between mb-2">
                    ...
                    <div class="group bg-gradient-to-br from-indigo-50 to-blue-50 rounded-xl p-4 border border-indigo-200 shadow-sm">
                      <div class="flex items-center justify-between mb-2">
                        <div class="text-xs text-indigo-700 font-medium">本月服务费</div>
                        <DollarSign :size="16" class="text-indigo-500" />
                      </div>
                      <div class="text-2xl font-bold text-indigo-700">¥{{ (emp.serviceRecords || []).reduce((sum, r) => sum + r.amount, 0) }}</div>
                    </div>
                  </div>
                      ...
                      <div v-for="record in emp.serviceRecords" :key="record.id" class="px-4 py-3 hover:bg-slate-50">
                        <div class="flex items-center justify-between">
                          ...
                          <div class="flex items-center gap-6 text-sm">
                            ...
                            <div class="text-center">
                              <div class="text-xs text-slate-500">服务次数</div>
                              <div class="font-semibold text-slate-900">{{ record.sessionCount }}次</div>
                            </div>
                            <div class="text-right">
                              <div class="text-xs text-slate-500">本月费用</div>
                              <div class="font-bold text-indigo-600">¥{{ record.amount }}</div>
                            </div>
                          </div>
                        </div>
                      </div>
...
import { Search, UserPlus, Edit, Trash2, ChevronDown, ChevronUp, Users, Sparkles, Clock, DollarSign, Target } from 'lucide-vue-next'
