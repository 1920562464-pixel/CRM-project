    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
      <table class="w-full">
        <thead class="bg-slate-50 border-b border-slate-200">
          <tr>
            <th class="px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase">员工</th>
            <th class="px-6 py-3 text-right text-xs font-semibold text-slate-600 uppercase">用户数</th>
            <th class="px-6 py-3 text-right text-xs font-semibold text-slate-600 uppercase">新用户</th>
            <th class="px-6 py-3 text-right text-xs font-semibold text-slate-600 uppercase">老用户</th>
            <th class="px-6 py-3 text-right text-xs font-semibold text-slate-600 uppercase">绩效奖金</th>
            <th class="px-6 py-3 text-right text-xs font-semibold text-slate-600 uppercase">合计</th>
        ...
        <tbody class="divide-y divide-slate-200">
          <tr v-for="settlement in settlements" :key="settlement.id" class="hover:bg-slate-50">
            <td class="px-6 py-4 font-medium text-slate-900">{{ settlement.employeeName }}</td>
            <td class="px-6 py-4 text-right text-slate-600">{{ settlement.totalUsers }}</td>
            <td class="px-6 py-4 text-right text-sm">
              <div class="text-slate-900">{{ settlement.newUserAmount }}</div>
              <div class="text-xs text-slate-500">({{ settlement.newUsers }}人)</div>
            </td>
            <td class="px-6 py-4 text-right text-sm">
              <div class="text-slate-900">{{ settlement.oldUserAmount }}</div>
              <div class="text-xs text-slate-500">({{ settlement.oldUsers }}人)</div>
            </td>
            <td class="px-6 py-4 text-right text-sm">{{ settlement.performanceBonus }}</td>
            ...
            <td class="px-6 py-4 text-center">
              <span :class="`px-2 py-1 rounded-full text-xs font-medium ${
                ...
                {{ settlement.status === 'pending' ? '待审批' : settlement.status === 'approved' ? '已审批' : settlement.status === 'paid' ? '已发放' : '已驳回' }}
              </span>
            </td>
            <td class="px-6 py-4">
...
import { Search, Plus, CheckCircle, XCircle, CreditCard } from 'lucide-vue-next'
